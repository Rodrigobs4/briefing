...
export default function DashboardExecutivo() {
    ...
    useEffect(() => {
        if (!isInitialized && units.length > 0) {
            if (user?.role === 'editor') {
                ...
                const editorUnits = (user.unitIds && user.unitIds.length > 0) ? user.unitIds : (user.unitId ? [user.unitId] : []);
                setSelectedUnitIds(editorUnits);
            } else {
    ...
    const hierarchicalView = useMemo(() => {
        ...
        unitsToProcess.forEach(unit => {
            ...
            const unitCollections = collectionItems.filter(i => i.unitId === unit.id && i.status !== 'archived');
            ...
            if (unitEntries.length > 0) {
                const latest = [...unitEntries].sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime())[0];
                latestUpdatedAt = new Date(latest.updatedAt).getTime();
                updatedByName = users.find(u => u.id === latest.updatedBy)?.name || updatedByName;
            }
            if (unitCollections.length > 0) {
                const latest = [...unitCollections].sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime())[0];
            ...
            const unitView = {
                unitId: unit.id,
                unitName: unit.name,
                unitFullName: unit.full_name,
                unitIcon: unit.description || 'briefcase',
                lastUpdated: latestUpdatedAt > 0 ? new Date(latestUpdatedAt).toISOString() : null,
            ...
            dataGroups.forEach(group => {
                if (group.mode === 'collection') {
                    const items = unitCollections.filter(i => i.dataGroupId === group.id);
                    if (items.length > 0) {
                        // Verifica se este grupo possui campo do tipo imagem configurado
                        const hasImageField = fields.some(f => f.dataGroupId === group.id && f.type === 'image' && f.isActive);

                        const processedItems = items.map(item => {
                            const itemValues = getValuesForItem(item.id);
                            const author = users.find(u => u.id === item.createdBy)?.name || 'Sistema';
                            ...
                            try {
                                if (imageField?.valueJson) {
                                    if (typeof imageField.valueJson === 'string' && imageField.valueJson.startsWith('[')) {
                        ...
                        processedItems.sort((a, b) => b.isFeatured === a.isFeatured ? b.dateRaw - a.dateRaw : (a.isFeatured ? -1 : 1));
                    ...
                    const entry = unitEntries.find(e => e.dataGroupId === group.id);
                    if (entry) {
                        const snapshotValues = getValuesForEntry(entry.id);
                        const groupFields = fields.filter(f => f.dataGroupId === group.id && f.isActive);

                        const numberFields = groupFields
                            .filter(field => ['number', 'percentage', 'calculated'].includes(field.type))
                            .map(field => {
                                ...
                                if ((val === null || val === undefined || val === '') && field.type === 'calculated') {
                                    const formDataForCalc = snapshotValues.reduce((acc, curr) => {
                                        acc[curr.fieldId] = curr.value;
                                        return acc;
                                    }, {} as Record<string, any>);

                                    const calculated = calculateFieldValue(field, formDataForCalc, fields);
                                    if (calculated !== null) {
                                        val = calculated;
                                    }
                                }
    ...
    }, [units, selectedUnitIds, entries, dataGroups, fields, getValuesForEntry, users, collectionItems, getValuesForItem]);
    ...
    const estatisticasResumo = useMemo(() => {
        ...
        const totalSnapshots = entries.filter(e => selectedUnitIds.includes(e.unitId)).length;
        const totalItens = collectionItems.filter(i => selectedUnitIds.includes(i.unitId) && i.status !== 'archived').length;
        const totalRelevantes = collectionItems.filter(i => selectedUnitIds.includes(i.unitId) && i.isFeatured && i.status !== 'archived').length;

        return {
            totalUnidades,
            totalRegistros: totalSnapshots + totalItens,
            totalRelevantes,
            atividadeGlobal: totalSnapshots > 0 || totalItens > 0 ? 'Frequente' : 'Sem Dados'
        };
        ...
        <div className="space-y-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-white p-6 rounded-3xl border border-pm-secondary/15 shadow-sm hover:shadow-premium transition-all flex items-center gap-5">
                    ...
                    <div>
                        <p className="text-[10px] font-black text-pm-secondary/50 uppercase tracking-widest leading-none mb-1">Total de Registros</p>
                        <h4 className="text-2xl font-black text-pm-dark leading-none">{estatisticasResumo.totalRegistros}</h4>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-3xl border border-pm-secondary/15 shadow-sm hover:shadow-premium transition-all flex items-center gap-5">
                    ...
                    <div>
                        <p className="text-[10px] font-black text-pm-secondary/50 uppercase tracking-widest leading-none mb-1">Itens Relevantes</p>
                        <h4 className="text-2xl font-black text-pm-dark leading-none">{estatisticasResumo.totalRelevantes}</h4>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-3xl border border-pm-secondary/15 shadow-sm hover:shadow-premium transition-all flex items-center gap-5">
                    ...
                    <div>
                        <p className="text-[10px] font-black text-pm-secondary/50 uppercase tracking-widest leading-none mb-1">Unidades Ativas</p>
                        <h4 className="text-2xl font-black text-pm-dark leading-none">{estatisticasResumo.totalUnidades}</h4>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-3xl border border-pm-secondary/15 shadow-sm hover:shadow-premium transition-all flex items-center gap-5">
                    ...
                    <div>
                        <p className="text-[10px] font-black text-pm-secondary/50 uppercase tracking-widest leading-none mb-1">Status da Atividade</p>
            ...
            <div className="space-y-10">
                {hierarchicalView.length === 0 && (
                    <div className="py-12 text-center bg-pm-light/50 border border-dashed border-pm-secondary/30 rounded-xl">
                        ...
                        <p className="text-pm-secondary font-medium">Nenhum indicador numérico preenchido pelas unidades ainda.</p>
                    </div>
                )}
                    ...
                    <div key={unitNode.unitId} className="flex flex-col gap-8 relative">
                        ...
                        <div className="border-b-2 border-pm-primary/20 pb-4">
                            <h2 className="text-2xl sm:text-3xl font-black text-pm-dark tracking-tighter uppercase flex items-center gap-4">
                                ...
                                <span>
                                    {unitNode.unitName} 
                                    {unitNode.unitFullName && <span className="hidden md:inline font-bold opacity-40 ml-2">— {unitNode.unitFullName}</span>}
                                </span>
                            </h2>
                            {unitNode.lastUpdated && (
                                <p className="text-[11px] text-pm-secondary font-bold flex items-center gap-2 mt-4 ml-16 uppercase tracking-wider">
                                    ...
                                    <span className="text-pm-dark/80">{new Date(unitNode.lastUpdated).toLocaleString('pt-BR')}</span>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                            {unitNode.groups.map(groupNode => {
                                ...
                                const fieldCount = (groupNode.numberFields?.length || 0) + (groupNode.textFields?.length || 0);
                                const isCollection = groupNode.mode === 'collection';
                                const hasLongText = groupNode.textFields?.some((f: any) => f.type === 'textarea' || (String(f.value).length > 50));
                                ...
                                const isSmallTopic = !isCollection && !hasLongText && fieldCount <= 2;

                                // 3. Definir classes de grid
                                const containerColSpan = isSmallTopic ? 'col-span-1' : 'md:col-span-2';
                                ...
                                return (
                                    ...
                                    >
                                        <div className="px-6 py-4 border-b border-pm-secondary/10 bg-pm-light/30">
                                            <h3 className="section-title mb-0">{groupNode.groupTitle}</h3>
                                        </div>
                                        <div className="p-6 flex-1">
                                            ...
                                            {groupNode.mode === 'snapshot' && (() => {
                                                ...
                                                let internalGridCols = isSmallTopic ? 'grid-cols-2' : 'grid-cols-2 sm:grid-cols-3 lg:grid-cols-4';

                                                // AJUSTE 1: OPMS - Grid de 3 colunas fixas no desktop (Harmonia 33%)
                                                if (groupNode.groupTitle === 'OPMS') {
                                                    internalGridCols = 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3';
                                                }

                                                // AJUSTE 2: CPMS - Grid de 4 colunas para permitir composição 25% / 75%
                                                const isCPMsTopic = groupNode.groupTitle.trim().toUpperCase() === 'CPMS';
                                                if (isCPMsTopic) {
                                                ...
                                                return (
                                                    <div className={`grid ${internalGridCols} gap-3`}>
                                                        {allFields.map((field: any) => {
                                                            ...
                                                            const isLongField = field.type === 'textarea' || (field.type === 'text' && String(field.value).length > 25);
                                                            ...
                                                            let fieldColSpan = isLongField
                                                                ? (isSmallTopic ? 'col-span-2' : 'col-span-2 sm:col-span-3 lg:col-span-4')
                                                                : 'col-span-1';
                                                            ...
                                                            if (isCPMsTopic) {
                                                                ...
                                                                if (label === 'Capital' || label === 'Interior') {
                                                                    fieldColSpan = 'md:col-span-1';
                                                                }
                                                                else if (label === 'Unidades da Capital' || label === 'Unidades do Interior') {
                                                            ...
                                                            const isNumeric = ['number', 'percentage', 'calculated'].includes(field.type);

                                                            return (
                                                                <div key={field.fieldId} className={`bg-pm-light/30 p-4 lg:p-6 rounded-2xl border border-pm-secondary/10 hover:border-pm-primary/40 hover:bg-white transition-all relative flex flex-col group/card shadow-sm ${fieldColSpan}`}>
                                                                    {isNumeric && <div className="absolute left-0 top-4 bottom-4 w-1 bg-pm-primary rounded-r-full opacity-60 group-hover/card:opacity-100 transition-opacity"></div>}

                                                                    <h4 className="text-[10px] uppercase tracking-widest text-pm-secondary/70 font-black mb-3 pl-2 truncate" title={field.label}>{field.label}</h4>
                                                                    ...
                                                                    <div className="pl-2 text-pm-dark mt-auto">
                                                                        {isNumeric ? (
                                                                            <div className="flex items-baseline gap-1.5">
                                                                                ...
                                                                                {field.type === 'percentage' && <span className="text-lg font-black text-pm-primary">%</span>}
                                                                            </div>
                                                                        ) : (
                                                                            <p className="text-sm font-bold whitespace-pre-wrap leading-relaxed text-pm-dark/80">{field.value}</p>
                                                                        )}
                                                                    </div>
                                                                </div>
                                                            );
                                            ...
                                            {groupNode.mode === 'collection' && groupNode.collectionItems && (
                                                <div className="flex flex-col gap-6">
                                                    {groupNode.collectionItems.map((item: any) => {
                                                        const hasImage = !!item.imageUrl;

                                                        return (
                                                            <div key={item.id} className={`flex flex-col ${hasImage ? 'lg:flex-row' : ''} gap-6 bg-white p-6 rounded-2xl border transition-all hover:shadow-premium group/item ${item.isFeatured ? 'border-amber-200 bg-amber-50/30' : 'border-pm-secondary/15'}`}>
                                                                ...
                                                                <div className="flex-1 flex flex-col min-w-0">
                                                                    <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-3 mb-3">
                                                                        <h4 className="text-xl font-black text-pm-dark leading-snug tracking-tight">{item.title}</h4>
                                                                        <div className="flex flex-col sm:items-end flex-shrink-0">
                                                                            {item.isFeatured && <span className="badge-warning px-3 py-1 text-[10px]"><ShieldAlert className="w-3 h-3" /> Relevante</span>}
                                                                        </div>
                                                                    </div>
                                                                    <p className="text-sm font-medium text-pm-secondary/80 whitespace-pre-wrap leading-relaxed flex-1 italic">{item.description}</p>
                                                                    <div className="mt-6 pt-4 border-t border-pm-secondary/10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 text-[10px] uppercase tracking-widest font-black text-pm-secondary/60">
                                                                        <span className="flex items-center gap-2">
                                                                            ...
                                                                            Reportado: <span className="text-pm-dark/70 font-black">{item.dateStr}</span>
                                                                        </span>
                                                                        <span className="bg-pm-light px-2 py-1 rounded border border-pm-secondary/10">
                                                                            Por: <span className="text-pm-dark/70 font-black">{item.author}</span>
                                                                        </span>
                                                                    </div>
                                                                </div>
                                                            </div>
            ...
            {isReportModalOpen && <ReportBuilderModal onClose={() => setIsReportModalOpen(false)} />}
        </div>
    );
}
