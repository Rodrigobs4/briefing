...
if (!supabaseUrl || !supabaseAnonKey) {
    console.error('ERRO CRÍTICO: Variáveis de ambiente do Supabase (VITE_SUPABASE_URL, VITE_SUPABASE_ANON_KEY) não foram encontradas.');
    // Não lançamos throw aqui para não quebrar o build do Vite inteiramente, mas o app não funcionará sem isso.
    if (typeof window !== 'undefined') alert('Configuração do Supabase ausente. Verifique o arquivo .env');
}
